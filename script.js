...
cartButton.addEventListener("click", function () {
  ...
  let totalAmount = 0;
  let romanNumerals = ['i)', 'ii)', 'iii)', 'iv)', 'v)', 'vi)', 'vii)', 'viii)', 'ix)', 'x)']; // You can extend this as needed
  cart.forEach((item, index) => {
      const romanNumeral = romanNumerals[index] || `${index + 1})`; // Fallback to regular numbering if more than 10 items
      cartItems.innerHTML += `<li>&nbsp;&nbsp;&nbsp;&nbsp;${romanNumeral} ${item.title}</li>`;
      cartItems.style.listStyleType = 'none';
  ...
  wishlistButton.addEventListener("click", function () {
      wishlistItems.innerHTML = wishlist.map(item => `<li>&nbsp;&nbsp;&nbsp;&nbsp;${item}</li>`).join('');
